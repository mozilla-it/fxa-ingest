...
customer_record_table_ddl: str = ("\n"
                                 ...
                                 f"  locale            STRING({MAX_LOCALE_LENGTH}),\n"
                                  "  lang              STRING(10),\n"
                                 f"  last_seen_country STRING({MAX_COUNTRY_LENGTH}),\n"
                                  "  marketing_opt_in  BOOL,\n"
                                 f"  metrics_context   STRING({MAX_MC_LENGTH}),\n"
                                  "  last_activity     TIMESTAMP NOT NULL OPTIONS(allow_commit_timestamp=true)\n"
                                  ") PRIMARY KEY (fxa_id)")
...
def update_customer_country(event_unique_id, message_json, message_dict):
    # TODO: give this the same treatment as update_customer_last_activity?
    try:
        with spanner_database.batch() as batch:
            batch.update(
                table='customer_record',
                columns=('fxa_id', 'last_seen_country','last_activity'),
                values=[(
                        transform('uid', message_dict),
                        transform('country', message_dict),
                        transform('ts', message_dict)
                        )]
            )
    except exceptions.NotFound as e:
        logging.debug(
            f"update_customer_country failed. no record found in database. uid={message_dict['uid']} eui={event_unique_id}")


def update_customer_last_activity(event_unique_id, message_json, message_dict):
    global last_activity_cache, lac_locked
    fxa_id = transform('uid', message_dict)
    ts     = transform('ts', message_dict)
    last_activity_cache[fxa_id] = (fxa_id, ts)
    random_num: int = int(random.random() * 100) + 100
    if len(last_activity_cache) > random_num and not lac_locked:
        # FIXME: still a race condition here, but last_activity isn't that important
        lac_locked = True
        logging.info("Executing update_customer_last_activity_batch()")
        try:
            update_customer_last_activity_batch()
        except:
            lac_locked = False
            raise
        lac_locked = False
    elif lac_locked:
        logging.debug("lac_locked was True - blocking until it isn't?")
        retries = 0
        while lac_locked:
            time.sleep(2)
            retries += 1
            if retries > 10:
                logging.warning('update_customer_last_activity had to wait too long for lac_locked')
                break

def update_customer_last_activity_batch():
    # TODO: Handle failed inserts better
    # TODO: somehow make this only run once?
    global last_activity_cache, lac_locked
    for _ in range(100):
        try:
            lac_copy = list(last_activity_cache.values())
            if len(lac_copy) == 0:
                logging.debug('lac_copy was empty')
                return
            with spanner_database.batch() as batch:
                batch.update(
                    table='customer_record',
                    columns=('fxa_id', 'last_activity'),
                    values=lac_copy
                )
            logging.debug('update_customer_last_activity_batch succeeded')
            print('update_customer_last_activity_batch succeeded')
            last_activity_cache = {}
            return
        except exceptions.NotFound as e:
            logging.debug('Row missing from customer_record. Attempting to remove from last_activity_cache')
            print('Row missing from customer_record. Attempting to remove from last_activity_cache')
            missing_row = None
            m = re.search('Row \[(.*)\] in table customer_record is missing', str(e))
            if m:
                missing_row = m.group(1)
                print("removing missing row %s" % missing_row)
                try:
                    del(last_activity_cache[missing_row])
                except KeyError:
                    print('and now that key isn\'t there')
                    break
                continue
            else:
                logging.warning(
                    f"update_customer_last_activity_batch failed. no record found in database. e={str(e)}")
        break
    print("TRIED TOO MANY TIMES")

def update_customer_last_activity_old(event_unique_id, message_json, message_dict):
    try:
        with spanner_database.batch() as batch:
            batch.update(
                table='customer_record',
        ...
        logging.debug(
        f"update_customer_last_activity failed. no record found in database. uid={message_dict['uid']} eui={event_unique_id}")
...
def update_customer_email(event_unique_id, message_json, message_dict):
    try:
        with spanner_database.batch() as batch:
            batch.update(
                table='customer_record',
                columns=('fxa_id', 'email', 'last_activity'),
                values=[(
                    transform('uid',   message_dict),
    ...
    with spanner_database.batch() as batch:
        batch.update(
            ...
            columns=('insert_id', 'fxa_id', 'fxa_ts', 'email', 'service', 'create_ts',
                     'locale', 'lang', 'marketing_opt_in', 'metrics_context', 'last_activity'),
            values=[(
                event_unique_id,
                data['fxa_id'],
                data['fxa_ts'],
...
def insert_customer_record(event_unique_id, message_json, message_dict, stub_data=False):
    ...
    if stub_data:
        ...
        data_to_insert = {
            ...
            'last_activity':    message_dict['fxa_ts'],
        }
    else:
        data_to_insert = {
            'insert_id':         event_unique_id,
            ...
            'metrics_context':   transform('metrics_context',  message_dict),
            'last_activity':     transform('ts',               message_dict),
        }
    try:
...
def pubsub_callback(message):
    ...
    if payload_dict['event'] == 'delete':
        ...
        if payload_dict['event'] == 'verified':
            ...
            # this also counts as a service login ... ? No, let's not do that.
            ...
            if insert_service_login(event_unique_id, payload_json, payload_dict):
                if 'country' in payload_dict:
                    update_customer_country(event_unique_id, payload_json, payload_dict)

        elif payload_dict['event'] == 'device:create':
            if insert_device(event_unique_id, payload_json, payload_dict):
            ...
            if delete_device(event_unique_id, payload_json, payload_dict):
                # uncomment when historical events are loaded
                #update_customer_last_activity(event_unique_id, payload_json, payload_dict)
                pass
